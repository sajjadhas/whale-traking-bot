...
def _norm(txt:str)->str:
    ...
    if m: return f"https://hyperdash.info/trader/{m.group(1)}"
...
def _fetch_and_crop_bottom(url:str, bottom_ratio:float=0.40):
    for _ in range(3):
        ...
        if r.ok and r.headers.get("Content-Type","").startswith("image"):
            ...
            buf = io.BytesIO(); crop.save(buf, format="JPEG", quality=90); buf.seek(0)
            return buf.read()
        time.sleep(2)
    return None
...
async def on_addr(u:Update, c:ContextTypes.DEFAULT_TYPE):
    ...
    if img:
        ...
        await u.message.reply_text("اسکرین‌شات نشد. دوباره تلاش کن.")
    return ConversationHandler.END
